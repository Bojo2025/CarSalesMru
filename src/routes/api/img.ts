...
async function resolveFacebookItemImage(itemId: string): Promise<string | null> {
  const { fetchMarketplacePreviewImage, facebookSeedImageMap } = await import(
    "@/lib/facebook.server"
  );
  const seeded = facebookSeedImageMap().get(itemId);
  if (seeded) return seeded;
  return fetchMarketplacePreviewImage(
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (fbId) {
          if (!/^\d{8,}$/.test(fbId)) {
            return new Response("Bad facebook id", { status: 400 });
          }
          try {
            const imageUrl = await resolveFacebookItemImage(fbId);
            if (!imageUrl) return new Response("No preview", { status: 404 });
